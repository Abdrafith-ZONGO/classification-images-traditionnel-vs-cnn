...
from src.utils import tracer_courbes_apprentissage, tracer_matrice_confusion, tracer_courbe_roc, tracer_comparaison_globale

def executer_pipeline_complet(datasets_a_traiter, modeles_extracteurs, classifieurs_ml, epochs_cnn, forcer_extraction=False):
    """
    on lance tout le traitement pour les datasets et les modeles choisis.
    """
    resultats_comparatifs = []
    
    for nom_dataset in datasets_a_traiter:
        print("\n" + "="*80)
        print(f"=== DEBUT DU TRAITEMENT : {nom_dataset} ===")
        print("="*80)
        
        # --- PHASE 1 : ML classique avec extraction ---
        print("\n--- PHASE 1 : Extraction de Features et Classifieurs ML ---")
        ...
        for nom_extractor in modeles_extracteurs:
            try:
                ...
                print(f"[ERREUR] Extraction impossible pour {nom_dataset} avec {nom_extractor} : {e}")
                continue
            ...
            for nom_clf in classifieurs_ml:
                try:
                    ...
                    # Comptage des parametres du modele d'extraction utilise
                    ...
                    resultats_comparatifs.append({
                        ...
                        'Nb_Parametres': nb_params_ext
                    })
                    
                except Exception as e:
                    print(f"[ERREUR] Echec entrainement {nom_clf} sur {nom_dataset} ({nom_extractor}) : {e}")
                    
        # --- PHASE 2 : CNN personnalise ---
        print("\n--- PHASE 2 : Entrainement direct du CNN ---")
        try:
            ...
            print(f"[ERREUR] Echec du CNN sur {nom_dataset} : {e}")

    # --- PHASE 3 : Comparaison finale ---
    if resultats_comparatifs:
        ...
        print("\n" + "="*80)
        print("=== GENERATION DU GRAPHIQUE COMPARATIF GLOBAL ===")
        print("="*80)
        tracer_comparaison_globale(df_comparatif)
        ...
        print("Aucun resultat n'a ete calcule.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Pipeline Classification Images : Traditionnel vs CNN")
    parser.add_argument(
        "--dataset", 
        type=str, 
        choices=["all", "Covid19-XRAYS", "DTD", "Iris", "Wildfire"], 
        default="Iris", 
        help="Dataset a traiter (par defaut : Iris)"
    )
    parser.add_argument(
        "--epochs", 
        type=int, 
        default=15, 
        help="Nombre d'epoques pour le CNN (par defaut : 15)"
    )
    parser.add_argument(
        "--force", 
        action="store_true", 
        help="Forcer la re-extraction des features"
    )
